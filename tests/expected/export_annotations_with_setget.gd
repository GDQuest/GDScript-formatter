...
@export_range(0., 500., .1, "or_greater", "suffix:m") var getter_distance: float = 200.:
	get:
		return getter_distance

@export_range(0., 500., .1, "or_greater", "suffix:m") var both_distance: float = 200.:
	set(value):
		both_distance = value
	get:
...
@export_enum(
	"VeryLongFirstChoice",
	"VeryLongSecondChoice",
	"VeryLongThirdChoice",
	"VeryLongFourthChoice",
)
